...
export default function AgendamentosCRUD() {
  ...
  async function loadAll() {
    ...
    setErr("");
    try {
      const [ag, pac, prof, esp, loc] = await Promise.all([
  ...
  async function remove(id) {
    const ok = window.confirm("Remover este agendamento? Essa ação não pode ser desfeita.");
    if (!ok) return;
    ...
    <div>
      <div style={styles.pageHeader}>
        <div>
          ...
          <div style={styles.h2}>Gestão administrativa: editar, cancelar e remover agendamentos.</div>
        </div>
      ...
      {editing ? (
        <div style={styles.card}>
          <div style={styles.cardHeader}>
            <div style={styles.cardTitle}>Editar agendamento #{editing.id}</div>
            <div style={styles.cardSubtitle}>Atualize os dados e salve as alterações.</div>
          </div>
          ...
          <div style={styles.cardBody}>
            <div style={{ display: "grid", gridTemplateColumns: isNarrow ? "1fr" : "1fr 1fr", gap: 10 }}>
              <FieldSelect
                ...
                options={pacientes.map((p) => ({ value: String(p.id), label: `${p.nome} (#${p.id})` }))}
              />
              ...
              <FieldSelect
                ...
                options={profissionais.map((p) => ({ value: String(p.id), label: p.nome }))}
              />
              ...
              <FieldSelect
                ...
                options={locais.map((l) => ({ value: String(l.id), label: `${l.nome} — ${l.municipio}` }))}
              />

              <Field label="Início" type="datetime-local" value={form.inicio} onChange={(v) => setF("inicio", v)} />
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 980 }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><Td colSpan={9}>Nenhum agendamento encontrado.</Td></tr>
              ) : (
                filtered.map((a) => {
                  ...
                  return (
                    <tr key={a.id}>
                      ...
                      <Td>{prof}</Td>
                      <Td>{loc}</Td>
                      <Td>{a.modalidade === "TELEMEDICINA" ? "Telemedicina" : "Presencial"}</Td>
                      <Td>{a.status}</Td>
                      <Td>
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                          <button onClick={() => startEdit(a)} style={styles.smallBtn}>Editar</button>
                          <button onClick={() => cancelarPorStatus(a)} style={styles.smallBtnWarn}>Cancelar</button>
                          <button onClick={() => remove(a.id)} style={styles.smallBtnDanger}>Remover</button>
                        </div>
                      </Td>
                    </tr>
                  );
      ...
      <div style={styles.footnote}>Cancelar altera a situação para cancelado e “Remover” exclui o registro.</div>
    </div>
  );
}
...
function Td({ children, colSpan }) {
  return <td colSpan={colSpan} style={styles.td}>{children}</td>;
}
...
const styles = {
  ...
  label: { fontSize: 12, color: "#334155", fontWeight: 900, marginBottom: 6 },
  input: { width: "100%", padding: 10, borderRadius: 12, border: "1px solid #bfdbfe", background: "white", outline: "none" },
  select: { width: "100%", padding: 10, borderRadius: 12, border: "1px solid #bfdbfe", background: "white", outline: "none" },

  actionsRow: { display: "flex", gap: 10, marginTop: 12, flexWrap: "wrap" },
  primaryBtn: { padding: "10px 12px", borderRadius: 12, border: "1px solid #1d4ed8", background: "#1d4ed8", color: "white", fontWeight: 900, cursor: "pointer" },
  secondaryBtn: { padding: "10px 12px", borderRadius: 12, border: "1px solid #bfdbfe", background: "white", color: "#0f172a", fontWeight: 900, cursor: "pointer" },

  listHeader: { padding: 12, borderBottom: "1px solid #dbeafe", display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", flexWrap: "wrap" },
  search: { padding: 10, borderRadius: 12, border: "1px solid #bfdbfe", width: 520, maxWidth: "100%", outline: "none" },

  th: { textAlign: "left", padding: 10, fontSize: 12, color: "#334155", borderBottom: "1px solid #dbeafe", whiteSpace: "nowrap" },
  td: { padding: 10, borderBottom: "1px solid #eef6ff", fontSize: 13, color: "#0f172a", verticalAlign: "top", whiteSpace: "nowrap" },

  smallBtn: { padding: "6px 10px", borderRadius: 12, border: "1px solid #bfdbfe", background: "white", fontWeight: 900, cursor: "pointer" },
  smallBtnWarn: { padding: "6px 10px", borderRadius: 12, border: "1px solid #fed7aa", background: "#ffedd5", color: "#9a3412", fontWeight: 900, cursor: "pointer" },
  smallBtnDanger: { padding: "6px 10px", borderRadius: 12, border: "1px solid #fecaca", background: "#fff1f2", color: "#991b1b", fontWeight: 900, cursor: "pointer" },

  footnote: { marginTop: 12, color: "#334155", fontSize: 12, fontWeight: 700 },
};
